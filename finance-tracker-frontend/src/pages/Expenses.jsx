...
import { listExpenses, createExpense, updateExpense, deleteExpense } from '../api.jsx'
...
export default function Expenses() {
  ...
  const load = async () => {
    ...
    try { setItems(await listExpenses(token)) }
  ...
  const create = async (e) => {
    ...
    try {
      await createExpense(token, { ...form, amount: Number(form.amount) })
  ...
  const save = async (it) => {
    try { await updateExpense(token, it.id, it); await load() }
    catch (e) { alert(e.message) }
  }
      ...
      <div className="card mb-3 shadow-sm">
        <div className="card-body">
          ...
          <form className="row g-2" onSubmit={create}>
            ...
            <div className="col-md-3"><label className="form-label required">Category</label><input name="category" className="form-control" value={form.category} onChange={onChange} required /></div>
      ...
      <div className="card shadow-sm">
        <div className="card-body">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <h6 className="m-0">Your Expenses</h6>
            <button className="btn btn-outline-secondary btn-sm" onClick={load}>Refresh</button>
          </div>
          {loading ? <p>Loading…</p> : (
            <div className="table-responsive">
              <table className="table align-middle">
                <thead><tr><th>ID</th><th>Amount</th><th>Category</th><th>Date</th><th>Description</th><th></th></tr></thead>
                <tbody>
                  {items.map(it => (
                    <tr key={it.id}>
                      <td>{it.id}</td>
                      ...
                      <td><input className="form-control form-control-sm" value={it.category} onChange={e => it.category = e.target.value} /></td>
